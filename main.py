...
# --- નવું: ડાઉનલોડ કરવા માટેનું endpoint ---
...
# --- 3. File Encrypt/Decrypt (સુધારેલું) ---
...
async def file_crypto(file: UploadFile = File(...), password: str = Form(...), action: str = Form(...)):
    ...
    try:
        ...
        # ડાઉનલોડ લિંક મોકલો
        return {"status": "success", "download_link": f"/download/{output_filename}"}
    except Exception:
        return {"status": "error", "message": "Decryption Failed! Wrong password."}
...
async def add_watermark(image: UploadFile = File(...), text: str = Form(...)):
    ...
    return {"status": "success", "download_link": f"/download/{out_filename}"}
